...
- A cascade that changes a package dependency edge, a seam, a lifecycle
  transition or a serve route updates `docs/ARCHITECTURE.md` in the same
  cascade; the module-graph block is regenerated by
  `scripts/arch-graph.mjs`, never edited by hand (check: docs.arch.stale)
