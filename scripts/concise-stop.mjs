...
let transcriptPath;
try { transcriptPath = JSON.parse(input).transcript_path; } catch { process.exit(0); }
...
for (let i = lines.length - 1; i >= 0; i--) {
  ...
  if (!lastAssistantText && m.role === "assistant" && Array.isArray(m.content)) {
    lastAssistantText = m.content.filter((c) => c.type === "text").map((c) => c.text).join("\n");
  } else if (m.role === "user") {
...
if (count > LIMIT) {
  console.error(`concise gate: final reply is ${count} non-empty lines (limit ${LIMIT}; target 6). Shorten it — outcome first, one line per point, link records instead of restating. The Patron says "expand" to lift the gate.`);
  process.exit(2);
}
process.exit(0);
